...
check was introduced: **41 findings across 14 files
and 1,517 lines**. CI compares against that budget and fails only when a
...
|---|---:|---|---|---|
| `RS003` | 24 | error | typed errors | panic-based control flow (`unwrap`/`expect`/`panic!`) |
| `RS001` | 6 | warn | immutable values | mutable local binding (`let mut`) |
| `RS007` | 6 | warn | effects pushed outward | direct stdout/stderr effect in library code |
...
*typed errors* · 24 occurrences at baseline
...
*effects pushed outward* · 6 occurrences at baseline
